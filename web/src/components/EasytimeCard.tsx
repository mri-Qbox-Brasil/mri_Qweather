...
export function EasytimeCard() {
  const c = useEasytime()
  const { visible, values, close } = c
  const audioRef = useRef<HTMLAudioElement>(null)
  const { pos, dragRef, onHandlePointerDown } = useDraggable()

  // Lua → NUI: tocar a sirene do tsunami.
  ...
  // Fechar com ESC (igual ao script.js original).
  useEffect(() => {
    if (!visible) return
  ...
  // <audio> sempre montado para o playsound funcionar mesmo com a tela fechada.
  const audio = (
  ...
  return (
    <>
      ...
      >
        <MriCard className="overflow-hidden rounded-2xl border-0 shadow-2xl shadow-black/50">
          {/* Header arrastável (alça de drag) */}
          <div
          ...
          <MriCardFooter className="gap-2 px-5 pb-5 pt-0">
            <MriButton variant="ghost" size="sm" className="mr-auto" onClick={close}>
              Fechar
            </MriButton>
